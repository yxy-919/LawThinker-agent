...
@register_class(alias='J1Bench.Scenario.DD')
class DD:
    ...
    def add_parser_args(parser):
        parser.add_argument("--case_database", default = "./src/data/case/J1-Eval_DD.jsonl", type=str)
        ...
        parser.add_argument("--max_conversation_turn", default=30, type=int, help="max conversation turn between the lawyer and the specific_character")
        parser.add_argument("--save_path", default="./src/data/dialog_history/32b/DD_dialog_history.jsonl", help="save path for dialog history")
        parser.add_argument("--max_workers", default=100, type=int, help="max workers for parallel DD")
    ...
    def remove_processed_cases(self):
        ...
        for i, specific_character in enumerate(self.case_pair[::-1]):
    ...
    async def _consult(self, specific_character, lawyer, semaphore):
        ...
        for turn in range(self.max_conversation_turn):
            ...
            print(dialog_history[-1]["turn"], dialog_history[-1]["role"])
            print(dialog_history[-1]["content"])
            if '询问结束' in lawyer_response_text or lawyer_response is None:
                break
